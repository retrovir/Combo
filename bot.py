...
class PokebotMultiClient:
    def __init__(self, config):
        ...
        self.inactivity_timeout = 15  # For guessing logic
    ...
    async def auto_hunt_inactivity(self):
        while True:
            ...
            if not self.active:
                continue
            now = asyncio.get_event_loop().time()
            if self.last_event_time and (now - self.last_event_time > 5):
                if not self.last_hunt_time or (now - self.last_hunt_time > 5):
                    print(f"[{self.name}][INACTIVITY] 10s passed since last event, sending /hunt.")
    ...
    async def cmd_kill(self, event):
        ...
        # Start auto-hunt inactivity watchdog if not already running
    ...
    async def run(self):
        try:
            ...
            # Start guess inactivity watchdog

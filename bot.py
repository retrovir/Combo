...
class PokebotMultiClient:
    def __init__(self, config):
        ...
        # For guessing inactivity
        self.inactivity_timeout = 15
        self.guess_watchdog_task = None

        # For improved hunt logic
        self.last_seen_current_turn = False
        self.last_pokebot_activity = None
        self.last_hunt_sent_time = None
    ...
    # =============================
    # AUTO-HUNT LOGIC (IMPROVED)
    # =============================
    async def auto_hunt_inactivity(self):
        while self.active:
            await asyncio.sleep(1)
            now = asyncio.get_event_loop().time()
            # Inactivity: If no pokebot message for 15s, send hunt
            if self.last_pokebot_activity is not None:
                if (now - self.last_pokebot_activity) > 15:
                    if not self.last_hunt_sent_time or (now - self.last_hunt_sent_time > 5):
                        print(f"[{self.name}] 15s inactivity, sending /hunt.")
                        await self.send_hunt()
                        self.last_hunt_sent_time = now

    async def start_auto_hunt_loop(self):
    ...
    async def cmd_kill(self, event):
        ...
        await self.start_auto_hunt_loop()
    ...
    async def poke_event(self, event):
        now = asyncio.get_event_loop().time()
        self.last_pokebot_activity = now

        msg_has_current_turn = "Current turn" in event.raw_text

        # If we saw "Current turn" in the previous message, but not this one, battle just ended
        if self.last_seen_current_turn and not msg_has_current_turn:
            print(f"[{self.name}] Battle ended, sending /hunt immediately.")
            await self.send_hunt()
            self.last_hunt_sent_time = now

        self.last_seen_current_turn = msg_has_current_turn

        # --- The rest of your original poke_event logic below ---
        if "shiny" in event.raw_text.lower():
    ...
    async def poke_event_edited(self, event):
        ...
        # If we saw "Current turn" in the previous message, but not this one, battle just ended (edited event)
        if self.last_seen_current_turn and not msg_has_current_turn:
            print(f"[{self.name}] (Edit) Battle ended, sending /hunt immediately.")
            await self.send_hunt()
            self.last_hunt_sent_time = now

        self.last_seen_current_turn = msg_has_current_turn

        # --- The rest of your original poke_event_edited logic below ---
        if not self.active:
            return
        self._reset_event_timer()
        try:
            if "used" in event.raw_text:
                await asyncio.sleep(1)
                print(f"[{self.name}] Damage Dealt By Poke, So Again Clicked On First Move")
                await event.click(1,1)
            elif "missed" in event.raw_text:
                await asyncio.sleep(1)
                print(f"[{self.name}] Attack Missed, So Again Clicked On First Move")
                await event.click(1,1)
            elif "fainted" in event.raw_text or "fled" in event.raw_text:
                await asyncio.sleep(1.2)
                print(f"[{self.name}] Poke fainted/fled, sending /hunt IMMEDIATELY.")
                await self.send_hunt()
                self.last_hunt_sent_time = now
